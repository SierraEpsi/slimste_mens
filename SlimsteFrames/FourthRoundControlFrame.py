...
class FourthRoundControlFrame:
    ...
    def refresh_answers(self, game_state, question):
        ...
        if game_state != "PLAY" or question.get_answer(5).is_revealed():
            answer_label = tk.Label(master=self.upper_frame, text=question.get_answer(5).get_answer())
            answer_label.grid(row=0, column=1, padx=5, pady=5)
        else:
            answer_button = tk.Button(self.upper_frame, text=question.get_answer(5).get_answer(), command=lambda: self.reveal_answer(5))
            answer_button.grid(row=0, column=1, padx=5, pady=5)

        if game_state != "PLAY" or question.get_answer(6).is_revealed():
            answer_label = tk.Label(master=self.upper_frame, text=question.get_answer(6).get_answer())
            answer_label.grid(row=1, column=1, padx=5, pady=5)
        else:
            answer_button = tk.Button(self.upper_frame, text=question.get_answer(6).get_answer(), command=lambda: self.reveal_answer(6))
            answer_button.grid(row=1, column=1, padx=5, pady=5)

        if game_state != "PLAY" or question.get_answer(7).is_revealed():
            answer_label = tk.Label(master=self.upper_frame, text=question.get_answer(7).get_answer())
            answer_label.grid(row=2, column=1, padx=5, pady=5)
        else:
            answer_button = tk.Button(self.upper_frame, text=question.get_answer(7).get_answer(), command=lambda: self.reveal_answer(7))
            answer_button.grid(row=2, column=1, padx=5, pady=5)

        if game_state != "PLAY" or question.get_answer(8).is_revealed():
            answer_label = tk.Label(master=self.upper_frame, text=question.get_answer(8).get_answer())
            answer_label.grid(row=3, column=1, padx=5, pady=5)
        else:
            answer_button = tk.Button(self.upper_frame, text=question.get_answer(8).get_answer(), command=lambda: self.reveal_answer(8))
            answer_button.grid(row=3, column=1, padx=5, pady=5)

        if game_state != "PLAY" or question.get_answer(9).is_revealed():
            answer_label = tk.Label(master=self.upper_frame, text=question.get_answer(9).get_answer())
            answer_label.grid(row=4, column=1, padx=5, pady=5)
        else:
            answer_button = tk.Button(self.upper_frame, text=question.get_answer(9).get_answer(), command=lambda: self.reveal_answer(9))
            answer_button.grid(row=4, column=1, padx=5, pady=5)
